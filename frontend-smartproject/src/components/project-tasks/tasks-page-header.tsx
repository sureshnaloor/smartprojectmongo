...
import { TASK_CATALOG_TABS, type TaskCatalogTab } from "./constants";

interface TasksPageHeaderProps {
  projectId: string;
  projectName?: string;
  activeTab: TaskCatalogTab;
  onTabChange: (tab: TaskCatalogTab) => void;
  allCount?: number;
  myCount?: number;
  pendingCount?: number;
  overdueCount?: number;
...
export function TasksPageHeader({
  ...
  const counts: Record<TaskCatalogTab, number | undefined> = {
    all: allCount,
    my: myCount,
  ...
  return (
    <div className="bg-[var(--bg-cream)] px-6 pb-5 pt-6 lg:px-8">
      <p className="mb-2 kanban-body-sm text-[var(--text-secondary)]">
        ...
        {" / "}
        <span className="text-[var(--text-primary)]">Tasks</span>
      </p>

      <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div>
          <h1 className="hse-display-md text-[var(--text-primary)]">Tasks</h1>
          <div
        ...
        <div className="flex flex-wrap items-center gap-2.5">
          {onGenerateTasks && (
            <Button variant="outline" size="sm" className="gap-1.5" onClick={onGenerateTasks}>
