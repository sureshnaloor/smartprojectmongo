...
export const MASTER_MODAL_CONFIG: Record<GlobalMasterKey, MasterModalConfig> = {
  ...
  equipment: {
    ...
    fields: [
      ...
      { key: "code", label: "Equipment Code / Number", type: "text", required: true, placeholder: "EQP-CRN-001" },
      { key: "name", label: "Equipment Name", type: "text", required: true, fullWidth: true, placeholder: 'e.g., "Mobile Crane 50T"' },
      { key: "ownershipType", label: "Ownership Type", type: "select", required: true, options: [{ value: "Own", label: "Own Equipment" }, { value: "Rental", label: "Rental Equipment" }] },
      { key: "category", label: "Equipment Type / Category", type: "select", required: true, options: ["Air Compressor", "Backhoe Loader", "Crawler Crane", "Dump Truck", "Excavator", "Forklift", "Generator", "Loader", "Mobile Crane", "Tower Crane"].map((v) => ({ value: v, label: v })) },
      { key: "subcategory", label: "Subcategory / Spec", type: "select", required: true, options: ["Lifting", "Earthmoving", "Power Generation", "Pneumatic", "Haulage", "Material Handling"].map((v) => ({ value: v, label: v })) },
      { key: "mappedResourceId", label: "Mapped Global Resource Type", type: "select", fullWidth: true, placeholder: "Select Resource Type (e.g. Mobile Crane 50T)…" },
      ...
      { key: "manufacturer", label: "Manufacturer", type: "select", options: ["Liebherr", "Caterpillar", "Komatsu", "JCB", "Hitachi", "Volvo", "Terex", "Sany"].map((v) => ({ value: v, label: v })) },
      { key: "modelYear", label: "Model / Year", type: "text", placeholder: "320D - 2024" },
      { key: "capacity", label: "Capacity / Specs", type: "text", fullWidth: true, placeholder: "50 Tons / 200 HP" },
      { key: "status", label: "Status", type: "toggle", required: true, defaultValue: "active" },
      { key: "inactiveReason", label: "Inactive Reason", type: "select", options: [{ value: "exit", label: "Exit / Decommissioned" }, { value: "maintenance", label: "Maintenance / Service" }] },
      { key: "exitDate", label: "Exit / Decommission Date", type: "date" },
      { key: "maintenanceStartDate", label: "Maintenance Start Date", type: "date" },
      { key: "maintenanceEndDate", label: "Maintenance End Date", type: "date" },
  ...
  employees: {
    ...
    fields: [
      ...
      { key: "employeeNumber", label: "Employee Code / Number", type: "text", required: true, placeholder: "EPC-EMP-00001" },
      { key: "fullName", label: "Full Name", type: "text", required: true },
      { key: "empPosition", label: "Designation / Position", type: "select", required: true, options: ["Fitter", "Rigger-2", "Welder", "Electrician", "Technician", "Foreman", "Supervisor", "Engineer", "Manager", "Laborer"].map((v) => ({ value: v, label: v })) },
      { key: "department", label: "Department / Trade", type: "select", required: true, options: ["Piping", "Structural", "Electrical", "Mechanical", "Civil", "Instrumentation", "Management", "Engineering"].map((v) => ({ value: v, label: v })) },
      { key: "employmentType", label: "Employment Type", type: "select", required: true, options: [{ value: "Own", label: "Own Employee" }, { value: "Contract", label: "Contract" }, { value: "Agency", label: "Agency" }, { value: "Rental", label: "Rental Manpower" }] },
      { key: "mappedResourceId", label: "Mapped Global Resource Type", type: "select", fullWidth: true, placeholder: "Select Resource Type (e.g. Fitter, Rigger-2)…" },
      { key: "phone", label: "Phone Number", type: "text" },
      { key: "email", label: "Email Address", type: "text" },
      { key: "entryDate", label: "Joining / Entry Date", type: "date" },
      { key: "status", label: "Status", type: "toggle", required: true, defaultValue: "active" },
      { key: "inactiveReason", label: "Inactive Reason", type: "select", options: [{ value: "exit", label: "Exit / Resignation" }, { value: "temporary-leave", label: "Temporary Leave" }] },
