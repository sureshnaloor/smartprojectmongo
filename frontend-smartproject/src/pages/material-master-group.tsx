...
async function getGroups(): Promise<MaterialGroup[]> {
...
async function updateGroup(
    id: number,
    data: Partial<{ name: string; description?: string; materialTypeId: number }>
): Promise<MaterialGroup> {
...
export default function MaterialMasterGroup() {
    const { toast } = useToast();
    const queryClient = useQueryClient();
    const [isDialogOpen, setIsDialogOpen] = useState(false);
    const [editingItem, setEditingItem] = useState<MaterialGroup | null>(null);
    ...
    const createMutation = useMutation({
        ...
        onError: (error: Error) => {
            toast({ title: error.message || "Error creating Material Group", variant: "destructive" });
        },
    });

    const updateMutation = useMutation({
        mutationFn: ({ id, data }: { id: number; data: Partial<{ name: string; description?: string; materialTypeId: number }> }) =>
            updateGroup(id, data),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ["/api/material-groups"] });
        ...
        onError: (error: Error) => {
            toast({ title: error.message || "Error updating Material Group", variant: "destructive" });
        },
    });
    ...
    const deleteMutation = useMutation({
        ...
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ["/api/material-groups"] });
            toast({ title: "Material Group deleted successfully" });
        },
        onError: (error: Error) => {
            toast({ title: error.message || "Error deleting Material Group", variant: "destructive" });
        },
    });

    const resetForm = () => {
        setFormData({ name: "", description: "", materialTypeId: "" });
        setEditingItem(null);
    };
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (!formData.materialTypeId) {
            toast({ title: "Please select a material type", variant: "destructive" });
            return;
        }
        const payload = {
            name: formData.name,
            description: formData.description || undefined,
            materialTypeId: parseInt(formData.materialTypeId, 10),
        };
        if (editingItem) {
    ...
    const handleEdit = (item: MaterialGroup) => {
        ...
        setFormData({
            ...
            materialTypeId: item.materialTypeId ? String(item.materialTypeId) : "",
        });
    ...
    const filteredItems = groups.filter((item) => {
        const typeName = typeNameById.get(item.materialTypeId) ?? "";
        ...
        <div className="h-full">
            <div className="p-8 min-h-screen" style={{
                ...
                <div className="max-w-7xl mx-auto">
                    <div className="mb-8">
                        <h1 className="text-2xl font-extrabold tracking-tight bg-gradient-to-r from-primary to-primary/60 bg-clip-text text-transparent mb-2">Material Group</h1>
                        <p className="text-gray-600">Manage material groups under each material type.</p>
                    </div>
                    ...
                    <div className="bg-white rounded-lg shadow p-4 mb-6">
                                ...
                                >
                                    <DialogHeader>
                                        <DialogTitle className="text-2xl font-semibold bg-gradient-to-r from-green-700 to-blue-600 bg-clip-text text-transparent">
                                            {editingItem ? "Edit Material Group" : "Add New Material Group"}
                                        </DialogTitle>
                                    </DialogHeader>
                                    <form onSubmit={handleSubmit} className="space-y-4">
                                        <div>
                                            <Label htmlFor="materialTypeId" className="font-semibold text-green-700">Material Type *</Label>
                                            <Select
                                                value={formData.materialTypeId || undefined}
                                                onValueChange={(value) =>
                        ...
                        {types.length === 0 && (
                            <p className="text-sm text-amber-700 mt-3">
                                Add at least one material type before creating groups.
                            </p>
                        )}
                    </div>
                            ...
                            <Table>
                                <TableHeader>
                                    <TableRow className="bg-gradient-to-r from-gray-100 to-slate-100 border-b-2 border-gray-200">
                                        <TableHead className="text-gray-900 font-bold">Material Type</TableHead>
                                    ...
                                    {filteredItems.map((item, index) => (
                                        ...
                                        >
                                            <TableCell className="text-green-800 font-medium">
                                                {typeNameById.get(item.materialTypeId) ?? (
                                                    <span className="text-gray-400 italic">Unknown type</span>
                                                )}
                                            </TableCell>
