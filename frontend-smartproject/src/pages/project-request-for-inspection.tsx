...
import { Loader2, Upload, FileText, Download, Eye, Trash2, ClipboardCheck } from "lucide-react";
...
export default function ProjectRequestForInspection() {
    ...
    const { data: rfiFiles, isLoading } = useQuery<RfiFile[]>({
        queryKey: [`/api/projects/${projectId}/request-for-inspection`],
    });
    ...
    const uploadMutation = useMutation({
        ...
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: [`/api/projects/${projectId}/request-for-inspection`] });
            toast({
                title: "Success",
                description: "RFI document uploaded successfully",
            });
            setUploading(false);
            setRfiName("");
            setDescription("");
    ...
    const deleteMutation = useMutation({
        mutationFn: async ({ fileId, fileName }: { fileId: string; fileName: string }) => {
            ...
            const res = await apiRequest("DELETE", `/api/projects/${projectId}/request-for-inspection?fileId=${fileId}&fileName=${encodedFileName}`);
            if (!res.ok) {
        ...
        onSuccess: () => {
            ...
            toast({
                title: "Success",
                description: "RFI document deleted successfully",
            });
        },
        onError: (error: Error) => {
            toast({
                title: "Error",
                description: "Failed to delete RFI document",
                variant: "destructive",
            });
        },
    });
    ...
    const handleFileUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (!rfiName) {
            toast({
                ...
                description: "Please enter an RFI Name before uploading.",
                variant: "destructive",
            });
            event.target.value = '';
            return;
    ...
    const handleDelete = (fileId: string, fileName: string) => {
        if (confirm("Are you sure you want to delete this RFI document?")) {
    ...
    const handleDownload = async (fileId: string, fileName: string) => {
        try {
            window.location.href = `/api/projects/${projectId}/request-for-inspection/download?fileId=${fileId}`;
        } catch (error) {
    ...
    const handleView = (fileId: string, fileName: string) => {
        // For viewing, we can also use the download route but maybe open in new tab if it's a PDF
        window.open(`/api/projects/${projectId}/request-for-inspection/download?fileId=${fileId}`, '_blank');
    };
        ...
        <div className="space-y-8 p-6 bg-background min-h-screen">
            ...
            <div className="relative flex items-center justify-center mb-8 p-6 rounded-xl bg-card border shadow-[0_10px_20px_rgba(0,0,0,0.1),0_6px_6px_rgba(0,0,0,0.1)] transition-all hover:shadow-[0_14px_28px_rgba(0,0,0,0.15),0_10px_10px_rgba(0,0,0,0.12)]">
                <div className="text-center">
                    ...
                    <p className="text-muted-foreground mt-2">Manage Project RFI Documents</p>
                </div>
                    ...
                    <div className="flex items-center justify-center w-full">
                        <label htmlFor="file-upload" className="flex flex-col items-center justify-center w-full h-32 border-2 border-dashed rounded-lg cursor-pointer bg-muted/5 hover:bg-muted/10 transition-colors">
                            <div className="flex flex-col items-center justify-center pt-5 pb-6">
                                ...
                                <p className="text-xs text-muted-foreground">PDF, Images, Word (MAX. 10MB)</p>
                            </div>
                            <input
                                id="file-upload"
                                type="file"
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    ...
                    {rfiFiles?.length === 0 && (
                        <div className="col-span-full flex flex-col items-center justify-center p-12 text-muted-foreground border-2 border-dashed rounded-xl bg-muted/10">
                            ...
                            <p className="text-lg font-medium">No RFI documents uploaded yet</p>
                            <p className="text-sm">Upload a request for inspection document to get started</p>
                        </div>
                    )}
                </div>
            )}
