...
// API functions
async function getTitles(): Promise<EmployeeTitle[]> {
    const response = await fetch("/api/employee-titles");
    if (!response.ok) throw new Error("Failed to fetch titles");
    return response.json();
}

async function createTitle(data: Omit<EmployeeTitle, "id" | "createdAt" | "updatedAt">): Promise<EmployeeTitle> {
    const response = await fetch("/api/employee-titles", {
        method: "POST",
...
async function updateTitle(
    id: number,
    data: Partial<Omit<EmployeeTitle, "id" | "createdAt" | "updatedAt">>
): Promise<EmployeeTitle> {
    const response = await fetch(`/api/employee-titles/${id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(data),
    });
...
async function deleteTitle(id: number): Promise<void> {
    const response = await fetch(`/api/employee-titles/${id}`, {
        method: "DELETE",
    });
...
export default function EmployeeMasterTitle() {
    const { toast } = useToast();
    const queryClient = useQueryClient();
    const [isDialogOpen, setIsDialogOpen] = useState(false);
    const [editingItem, setEditingItem] = useState<EmployeeTitle | null>(null);
    ...
    const createMutation = useMutation({
        ...
        onError: () => {
            toast({ title: "Error creating title", variant: "destructive" });
        },
    });

    const updateMutation = useMutation({
        mutationFn: ({ id, data }: { id: number; data: Partial<Omit<EmployeeTitle, "id" | "createdAt" | "updatedAt">> }) =>
            updateTitle(id, data),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ["/api/employee-titles"] });
    ...
    const handleEdit = (item: EmployeeTitle) => {
        setEditingItem(item);
        setFormData({
            name: item.name,
        ...
        <EmployeeMasterLayout>
            <div className="h-full">
                <div className="p-8 min-h-screen" style={{
                    backgroundImage: 'linear-gradient(135deg, #f8fafc 0%, #f1f5f9 25%, #f0fdf4 50%, #eff6ff 75%, #f8fafc 100%)',
                }}>
                    <div className="max-w-7xl mx-auto">
                        {/* Header */}
                        <div className="mb-8">
                            <h1 className="text-3xl font-extrabold tracking-tight bg-gradient-to-r from-emerald-700 to-teal-600 bg-clip-text text-transparent mb-2">Title Master</h1>
                            <p className="text-zinc-600">Manage official employee titles (e.g. Mr, Mrs, Ms, Dr).</p>
                        </div>
                            ...
                            <div className="flex flex-col sm:flex-row items-center gap-4">
                                <div className="relative flex-1 w-full">
                                    <Input
                                        ...
                                        className="w-full bg-white border-zinc-200 focus:ring-emerald-500 rounded-lg pl-3"
                                    />
                                </div>
                                ...
                                <Dialog open={isDialogOpen} onOpenChange={(open) => {
                                    ...
                                    <DialogTrigger asChild>
                                        <Button className="w-full sm:w-auto bg-emerald-700 hover:bg-emerald-800 text-white shadow-md transition-all hover:scale-105">
                                    ...
                                    <DialogContent className="max-w-md overflow-hidden rounded-2xl p-0 border-none shadow-2xl">
                                        <div className="bg-gradient-to-br from-emerald-600 to-teal-700 p-6 text-white">
                                            ...
                                            <p className="text-emerald-100 text-sm mt-1">
                                                {editingItem ? "Modify the existing title details." : "Define a new title for employee profiles."}
                                            </p>
                                        </div>

                                        <form onSubmit={handleSubmit} className="p-6 bg-white space-y-5">
                                            <div className="space-y-2">
                                                <Label htmlFor="name" className="text-sm font-semibold text-zinc-700">Title Name <span className="text-red-500">*</span></Label>
                                                <Input
                                                    id="name"
                                                    required
                                                    value={formData.name}
                                                    ...
                                                    placeholder="e.g. Mr, Dr, Eng"
                                                    className="focus:ring-emerald-500 border-zinc-200"
                                                />
                                            </div>
                                            ...
                                            <div className="space-y-2">
                                                ...
                                                <Textarea
                                                    ...
                                                    className="min-h-[100px] focus:ring-emerald-500 border-zinc-200"
                                                />
                                            </div>
                                            ...
                                            <div className="flex gap-3 pt-2">
                                                ...
                                                >
                                                    {editingItem ? "Save Changes" : "Create Title"}
                                                </Button>
                                            </div>
                                        </form>
                                    </DialogContent>
                        ...
                        <div className="bg-white/90 backdrop-blur-sm rounded-xl shadow-lg border border-zinc-200 overflow-hidden transition-all hover:shadow-xl">
                            {isLoading ? (
                                <div className="p-20 text-center">
                                    ...
                                    <p className="mt-4 text-zinc-500 font-medium">Loading titles...</p>
                                </div>
                                ...
                                <div className="p-20 text-center">
                                    ...
                                    <p className="text-zinc-500 font-medium">No titles found matching your search.</p>
                                    <Button
                                        variant="link"
                                ...
                                <Table>
                                    <TableHeader>
                                        <TableRow className="bg-zinc-50/80 border-b border-zinc-200">
                                            <TableHead className="text-zinc-900 font-bold py-4">Title Name</TableHead>
                                    ...
                                    <TableBody>
                                        {filteredItems.map((item, index) => (
                                            <TableRow
                                                key={item.id}
                                                className={`group transition-colors duration-200 border-b border-zinc-100 hover:bg-emerald-50/50 ${index % 2 === 0 ? "bg-white" : "bg-zinc-50/30"}`}
                                            >
                                                <TableCell className="font-semibold text-emerald-800 py-4 capitalize">{item.name}</TableCell>
