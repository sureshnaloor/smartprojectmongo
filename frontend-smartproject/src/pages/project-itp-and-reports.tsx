...
import { Loader2, Upload, FileText, Download, Eye, Trash2, FileCheck } from "lucide-react";
...
export default function ProjectItpAndReports() {
    ...
    const { data: files, isLoading } = useQuery<DocFile[]>({
        queryKey: [`/api/projects/${projectId}/itp-and-reports`],
    });
    ...
    const uploadMutation = useMutation({
        ...
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: [`/api/projects/${projectId}/itp-and-reports`] });
            toast({
                title: "Success",
    ...
    const deleteMutation = useMutation({
        mutationFn: async ({ fileId, fileName }: { fileId: string; fileName: string }) => {
            ...
            const res = await apiRequest("DELETE", `/api/projects/${projectId}/itp-and-reports?fileId=${fileId}&fileName=${encodedFileName}`);
            if (!res.ok) {
    ...
    const handleDownload = async (fileId: string, fileName: string) => {
        try {
            window.location.href = `/api/projects/${projectId}/itp-and-reports/download?fileId=${fileId}`;
        } catch (error) {
    ...
    const handleView = (fileId: string, fileName: string) => {
        window.open(`/api/projects/${projectId}/itp-and-reports/download?fileId=${fileId}`, '_blank');
    };
        ...
        <div className="space-y-8 p-6 bg-background min-h-screen">
            ...
            <div className="relative flex items-center justify-center mb-8 p-6 rounded-xl bg-card border shadow-[0_10px_20px_rgba(0,0,0,0.1),0_6px_6px_rgba(0,0,0,0.1)] transition-all hover:shadow-[0_14px_28px_rgba(0,0,0,0.15),0_10px_10px_rgba(0,0,0,0.12)]">
                <div className="text-center">
                    ...
                    <p className="text-muted-foreground mt-2">Inspection Test Plans and Project Reports</p>
                </div>
                    ...
                    <div className="flex items-center justify-center w-full">
                        <label htmlFor="file-upload" className="flex flex-col items-center justify-center w-full h-32 border-2 border-dashed rounded-lg cursor-pointer bg-muted/5 hover:bg-muted/10 transition-colors">
                            <div className="flex flex-col items-center justify-center pt-5 pb-6">
                                ...
                                <p className="text-xs text-muted-foreground">PDF, Excel, Word, Images (MAX. 10MB)</p>
                            </div>
                            <input
                                id="file-upload"
                                type="file"
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    ...
                    {files?.length === 0 && (
                        <div className="col-span-full flex flex-col items-center justify-center p-12 text-muted-foreground border-2 border-dashed rounded-xl bg-muted/10">
                            ...
                            <p className="text-sm">Upload ITPs or reports to get started</p>
                        </div>
                    )}
                </div>
            )}
