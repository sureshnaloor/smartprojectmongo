...
// API functions
async function getTrades(): Promise<EmployeeTrade[]> {
    const response = await fetch("/api/employee-trades");
    if (!response.ok) throw new Error("Failed to fetch trades");
    return response.json();
}

async function createTrade(data: Omit<EmployeeTrade, "id" | "createdAt" | "updatedAt">): Promise<EmployeeTrade> {
    const response = await fetch("/api/employee-trades", {
        method: "POST",
...
async function updateTrade(
    id: number,
    data: Partial<Omit<EmployeeTrade, "id" | "createdAt" | "updatedAt">>
): Promise<EmployeeTrade> {
    const response = await fetch(`/api/employee-trades/${id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(data),
    });
...
export default function EmployeeMasterTrade() {
    const { toast } = useToast();
    const queryClient = useQueryClient();
    const [isDialogOpen, setIsDialogOpen] = useState(false);
    const [editingItem, setEditingItem] = useState<EmployeeTrade | null>(null);
    ...
    const createMutation = useMutation({
        ...
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ["/api/employee-trades"] });
            toast({ title: "Trade created successfully" });
            setIsDialogOpen(false);
            resetForm();
        },
        onError: () => {
            toast({ title: "Error creating trade", variant: "destructive" });
        },
    });

    const updateMutation = useMutation({
        mutationFn: ({ id, data }: { id: number; data: Partial<Omit<EmployeeTrade, "id" | "createdAt" | "updatedAt">> }) =>
            updateTrade(id, data),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ["/api/employee-trades"] });
        ...
        onError: () => {
            toast({ title: "Error updating trade", variant: "destructive" });
        },
    });
    ...
    const deleteMutation = useMutation({
        ...
        onError: () => {
            toast({ title: "Error deleting trade", variant: "destructive" });
        },
    });
    ...
    const handleEdit = (item: EmployeeTrade) => {
        setEditingItem(item);
        setFormData({
            name: item.name,
    ...
    const filteredItems = trades.filter((item) =>
        ...
        <EmployeeMasterLayout>
            <div className="h-full">
                <div className="p-8 min-h-screen" style={{
                    backgroundImage: 'linear-gradient(135deg, #f0f9ff 0%, #e0f2fe 25%, #f0fdf4 50%, #dcfce7 75%, #f0f9ff 100%)',
                }}>
                    <div className="max-w-7xl mx-auto">
                        {/* Header */}
                        <div className="mb-8">
                            <h1 className="text-3xl font-extrabold tracking-tight bg-gradient-to-r from-blue-700 to-green-600 bg-clip-text text-transparent mb-2">Trade Master</h1>
                            <p className="text-zinc-600">Register and manage skilled trades and specializations.</p>
                        </div>
                        ...
                        <div className="bg-white/80 backdrop-blur-sm rounded-xl shadow-lg border border-white/20 p-4 mb-6 transition-all hover:shadow-xl">
                            <div className="flex flex-col sm:flex-row items-center gap-4">
                                <div className="relative flex-1 w-full">
                                    <Input
                                        placeholder="Search by trade name or description..."
                                        value={searchTerm}
                                ...
                                <Dialog open={isDialogOpen} onOpenChange={(open) => {
                                    ...
                                    <DialogContent className="max-w-md overflow-hidden rounded-2xl p-0 border-none shadow-2xl">
                                        <div className="bg-gradient-to-br from-blue-600 to-green-700 p-6 text-white">
                                            <DialogTitle className="text-2xl font-bold">
                                                {editingItem ? "Edit Trade" : "Add New Trade"}
                                            </DialogTitle>
                                            <p className="text-blue-100 text-sm mt-1">
                                                {editingItem ? "Modify the existing trade skill." : "Define a new skilled trade or specialization."}
                                            </p>
                                        </div>

                                        <form onSubmit={handleSubmit} className="p-6 bg-white space-y-5">
                                            <div className="space-y-2">
                                                <Label htmlFor="name" className="text-sm font-semibold text-zinc-700">Trade Name <span className="text-red-500">*</span></Label>
                                                <Input
                                                    id="name"
                                                    required
                                                    value={formData.name}
                                            ...
                                            <div className="flex gap-3 pt-2">
                                                ...
                                                >
                                                    {editingItem ? "Save Changes" : "Create Trade"}
                                                </Button>
                                            </div>
                                        </form>
                                    </DialogContent>
                        ...
                        <div className="bg-white/90 backdrop-blur-sm rounded-xl shadow-lg border border-zinc-200 overflow-hidden transition-all hover:shadow-xl">
                            {isLoading ? (
                                <div className="p-20 text-center">
                                    ...
                                    <p className="mt-4 text-zinc-500 font-medium">Loading trades...</p>
                                </div>
                                ...
                                <div className="p-20 text-center">
                                    ...
                                    <p className="text-zinc-500 font-medium">No trades found matching your search.</p>
                                    <Button
                                        variant="link"
                                ...
                                <Table>
                                    <TableHeader>
                                        <TableRow className="bg-zinc-50/80 border-b border-zinc-200">
                                            <TableHead className="text-zinc-900 font-bold py-4">Trade Name</TableHead>
