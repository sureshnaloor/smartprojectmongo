...
interface AllocationRentalEquipmentResponse {
  mapped: MappedRow[];
  unmapped: MappedRow["equipment"][];
}

export default function AllocationRentalEquipment() {
  ...
  const { data, isLoading, isError, error } = useQuery<AllocationRentalEquipmentResponse>({
    queryKey: ["/api/allocation/rental-equipment"],
    queryFn: async () => {
      const res = await fetch("/api/allocation/rental-equipment");
      if (!res.ok) throw new Error("Failed to load rental equipment allocation");
      return res.json();
    },
  });
    ...
    <div className="flex-1 min-w-0 bg-zinc-50">
      ...
      <div className="p-4 sm:p-6 lg:p-8 max-w-[1200px] mx-auto">
          ...
          <CardContent className="pt-0">
            ...
            {!isLoading && !isError && (
              <div className="rounded-md border border-zinc-200 overflow-x-auto">
                          ...
                          <TableRow className={cn(open && "bg-zinc-50/50")}>
                            ...
                            <TableCell className="py-1">
                              <RentalEquipmentResourceMapper
                                rentalEquipmentId={eq.id}
                                equipmentDisplayName={eq.equipmentName}
                            ...
                            <TableRow className="bg-zinc-50/90 hover:bg-zinc-50/90">
                              <TableCell colSpan={8} className="p-0 border-t border-zinc-100">
                                <div className="px-4 py-4 pl-12 space-y-4">
                                  ...
                                  <AssignmentMonthCalendar
                                    assignments={row.assignments}
                                    emptyDateHint="No planned start/end dates. Set dates in Project → Resources for this rental equipment resource on each work package."
                                  />
                                </div>
                              </TableCell>
                            </TableRow>
                ...
                {mapped.length === 0 && (
                  <p className="text-sm text-zinc-500 p-4 text-center">No rental equipment mapped yet.</p>
                )}
              </div>
            )}
          </CardContent>
        ...
        <Card className="border-zinc-200 shadow-sm border-dashed">
          <CardHeader className="pb-3">
            <CardTitle className="text-sm font-bold text-zinc-800">Not mapped to a rental equipment resource</CardTitle>
            <p className="text-xs text-zinc-500 font-normal">Same as Rental Equipment master — Map Resource.</p>
          </CardHeader>
          <CardContent className="pt-0">
            {isLoading && (
              <div className="space-y-2">
            ...
            {!isLoading && !isError && (
              <div className="rounded-md border border-zinc-200 overflow-x-auto">
                ...
                {unmapped.length === 0 && (
                  <p className="text-sm text-zinc-500 p-4 text-center">All rental equipment is mapped.</p>
                )}
              </div>
            )}
          </CardContent>
          ...
          <TabsContent value="utilization">
            <ResourceUtilizationPanel
              ...
              emptyHint="No mapped rental equipment resources with assignment data yet."
            />
          </TabsContent>
        </Tabs>
      </div>
