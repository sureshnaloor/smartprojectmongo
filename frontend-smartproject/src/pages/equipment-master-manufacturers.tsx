...
async function createManufacturer(data: { name: string; description?: string }): Promise<Manufacturer> {
  const res = await fetch("/api/equipment-manufacturers", {
    method: "POST",
...
async function updateManufacturer(
  ...
  const res = await fetch(`/api/equipment-manufacturers/${id}`, {
    method: "PATCH",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(data),
  });
...
async function deleteManufacturer(id: number): Promise<void> {
  const res = await fetch(`/api/equipment-manufacturers/${id}`, { method: "DELETE" });
  if (!res.ok) throw new Error("Failed to delete");
}

export default function EquipmentMasterManufacturers() {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/equipment-manufacturers"] });
      toast({ title: "Manufacturer created successfully" });
      setIsDialogOpen(false);
      setFormData({ name: "", description: "" });
      setEditingItem(null);
    },
    onError: () => toast({ title: "Error creating manufacturer", variant: "destructive" }),
  });
  ...
  const updateMutation = useMutation({
    ...
    onError: () => toast({ title: "Error updating manufacturer", variant: "destructive" }),
  });
  ...
  const deleteMutation = useMutation({
    ...
    onError: () => toast({ title: "Error deleting manufacturer", variant: "destructive" }),
  });
      ...
      <div className="p-8 min-h-screen">
        <div className="max-w-4xl mx-auto">
          <div className="mb-8">
            ...
            <p className="text-muted-foreground mt-1">Manage equipment manufacturers and OEMs</p>
          </div>
            ...
            <div className="flex items-center gap-4">
              ...
              <Dialog open={isDialogOpen} onOpenChange={(o) => { setIsDialogOpen(o); if (!o) { setEditingItem(null); setFormData({ name: "", description: "" }); } }}>
                ...
                <DialogContent className="max-w-md">
                  <DialogHeader>
                    <DialogTitle>{editingItem ? "Edit Manufacturer" : "Add Manufacturer / OEM"}</DialogTitle>
                  </DialogHeader>
          ...
          <div className="bg-white rounded-lg shadow overflow-hidden">
            {isLoading ? (
              ...
              <div className="p-8 text-center text-muted-foreground">No manufacturers found</div>
            ) : (
              <Table>
                <TableHeader>
                  <TableRow>
                      ...
                      <TableCell>
                        <div className="flex gap-2">
                          ...
                          <Button
                            ...
                            onClick={() => confirm("Delete this manufacturer?") && deleteMutation.mutate(item.id)}
                          >
